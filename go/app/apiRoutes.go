...
func getItemById(c echo.Context) error {
	...
	stmt, err := db.Prepare("SELECT items.id, items.name, categories.category_name, items.image_name FROM items INNER JOIN categories ON items.category_id = categories.category_id WHERE id = ?")
	if err != nil {
		c.Logger().Fatalf("stmtを生成できませんでした %v", err)
	}
	defer func(data *sql.Stmt) {
